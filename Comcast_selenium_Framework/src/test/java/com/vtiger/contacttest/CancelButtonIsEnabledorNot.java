...
public class CancelButtonIsEnabledorNot {
	...
	public void fillcreatepage()
	{
		...
		boolean logo = driver.findElement(By.xpath("(//input[@title='Cancel [Alt+X]'])[1]")).isEnabled();	
		if(logo==true)
		{
			Reporter.log("Cancel button is enabled",true);
			driver.findElement(By.xpath("(//input[@title='Cancel [Alt+X]'])[1]")).click();
		}
		else
		{
			Reporter.log("Cancel button is disabled",true);
		}
